...
Summary extraction serves each document's complete source pages in one payload — PI extraction is not windowed. Page-intact adaptive windows build toward each category's character target with defaults of hearings 6,000 characters/six primary pages, reports 10,000 characters/ten primary pages, and minute orders 6,000 characters/six primary pages, all under the 12,000-character primary-source safety limit; a character target above the safety limit is normalized to it, and one oversized page remains intact. Those windows apply only to the direct-API paths — the prompt-testing window and minute-order summaries — where each window is a separate request, hearing windows prefer examination boundaries, and a preceding context-only page may be included. Window size is a batching control only and never a direct output-length rule. Narrative density in synthesis is guided by a soft words-per-report target (`summarize_reports_window_target_words`, default 250, `0` disables it): prompt guidance about output shape only — never an API token cap, truncation rule, validator, retry, or repair pass.
