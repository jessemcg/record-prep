...
- `recordprep/summary_agents.py`: two-stage PI summary pipeline — category schemas, work items, fingerprints, canonical Markdown digest/meta publication with per-kind locks, quote span resolution, recurrence analysis, synthesis validation, and deterministic rendering.
...
Each summary category has its own built-in prompt migration chain, so previously shipped built-ins — including the retired digest built-ins with length and six-quote guidance, recognized by exact historical text — advance to the current relevance, scope, schema, quote, and safety contract, while custom prompts remain byte-for-byte unchanged and are wrapped as lower-priority additional guidance that cannot override the immutable contract. Do not reinterpret the retired paragraph-count setting as a page count. The complete hearing and report guidance shown in Settings must explain their labeled input sections; do not append a hidden attribution contract. The hearing source payload includes validated participant-index context privately under `PARTICIPANT INDEX CONTEXT — FOR ATTRIBUTION ONLY` — counsel, participants, and the witness/testimony picture (verified witnesses with mapped, cited examinations, or explicit `Testimony: None.`) — as attribution context only; every submitted fact still requires evidence from original hearing pages. Summaries are nonauthoritative; source pages remain the factual authority.
