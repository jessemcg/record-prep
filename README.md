...
Extraction **transport windows** are adaptive, page-aligned, and configured per category (hearings 6,000 characters/six pages; reports 10,000/10; under the 12,000-character safety limit, examination breaks preferred for hearings). Within one document's session they are served in source order without repeating a preceding context page; they are batching controls only and never an output-length rule. A conservative detector excludes the formal package of proposed/recommended findings and orders from report extraction: material after the marker is clipped deterministically, evidence pages past the cutoff are rejected, and matched proposal text is never logged or persisted. Actual historical orders and high-level agency recommendations remain in scope.
...
Extraction **transport windows** are adaptive, page-aligned **ephemeral** batches configured per category in Settings. RecordPrep adds complete source pages toward each category's character target and stops at its maximum page count or the 12,000-character primary-source safety limit (targets above the safety limit are normalized to it), while one oversized source page always remains intact. Hearings and minute orders default to a 6,000-character target and six primary pages; reports default to a 10,000-character target and ten primary pages. Hearing windows additionally prefer a break before a mapped witness examination. Within one document's PI session the windows are served in source order and every primary page is exposed exactly once; the window text itself is staged only in the child's private, disposable workspace and never persisted in the bundle. Window size is a batching control only: it never implies an output-length rule.

Hearing extraction windows privately include validated counsel/participant/examination context under `PARTICIPANT INDEX CONTEXT — FOR ATTRIBUTION ONLY`. It is attribution context only: every submitted fact still requires evidence from the original hearing pages, and the extraction skill explains that the metadata never replaces the transcript as the factual source.

Report extraction additionally excludes the formal package of proposed or recommended advisements, findings, and orders (with associated boilerplate) offered for court adoption. The existing conservative detector recognizes only bounded structural signatures — a `PROPOSED`/`RECOMMENDED FINDINGS AND ORDERS` title, a formal lead-in asking the court to make the following findings and orders, a split proposed-findings-then-proposed-orders template, or a proposal expressly asking the court to `find`/`order` — and the window payload marks the page with an in-text scope delimiter plus a `REPORT PROPOSAL EXCLUSION CONTEXT — FOR SCOPE ONLY` note, without deleting or rewriting any source text. Python rejects any evidence quote from pages past the cutoff, keeps actual historical orders and high-level agency recommendations eligible, and persists only fingerprints and counts — never matched proposal text.
