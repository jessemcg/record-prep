...
Summary inputs are adaptive, page-aligned **ephemeral windows** configured per category in Settings. RecordPrep adds complete source pages toward each category's character target and stops at its maximum page count or the 12,000-character primary-source safety limit (targets above the safety limit are normalized to it), while one oversized source page always remains intact. Hearings and minute orders default to a 6,000-character target and six primary pages; reports default to a 10,000-character target and ten primary pages. Hearing windows additionally prefer a break before a mapped witness examination. The preceding page may be sent as context-only. Every primary page is summarized exactly once; no final compression pass discards unique detail, and no window text or metadata is written to disk. Window size is a batching control only: it changes how many source pages share a window and can make individual paragraphs denser, so output length is controlled directly by the report density setting below.
...
**Report density** is controlled directly by a soft word target (`Report summary target (words per window)`, default 250; `0` disables it). When enabled, report requests carry an ephemeral `REPORT SUMMARY LENGTH GUIDANCE — FOR OUTPUT SHAPE ONLY` section stating the approximate target. It is prompt guidance about output shape only — never an API token cap, truncation rule, validator, retry, or repair pass — and RecordPrep never cuts off or mechanically rejects an answer because of it. The built-in report prompt explains the section and resolves over-density by prioritizing new or changed legally significant facts, observations, interviews, substantive assessments, procedural developments, and recommendations; synthesizing repeated history and substantially duplicative updates; omitting routine administrative detail unless it changes posture or bears materially on an issue; keeping conflicting accounts and attribution distinct; using fewer words than the target when the eligible material warrants less; and exceeding the target rather than ending mid-thought. Hearing and minute-order requests never include the guidance section.
...
Settings include source extraction, local OCR, classification/case inference, the Summarize API/prompts, per-category summary-window character targets and maximum page counts (hearings, reports, and minute orders, all normalized to the 12,000-character safety cap), the report word target per window (`0` disables the guidance), and PI command, model, and reasoning-level selection. The legacy shared `summarize_window_target_chars`/`summarize_window_max_pages` pair migrates to the per-category keys (a customized pair copies to every category; the untouched `6000`/`6` default initializes reports to `10000`/`10`) and is removed on the next Settings save. Numeric entries are validated before saving, and invalid input keeps the Settings window open with an actionable message. The PI reasoning level is stored in the project `.pi/settings.json`; choosing the global default removes the project override. Retired artifact-pipeline credentials and prompts — including the old cross-case `rt_ct_split_page` key — are removed when local config is loaded or saved. The transcript expander offers automatic detection plus case-local manual overrides; a stale global value never populates a new case. The obsolete paragraph-count setting is not reinterpreted as a page count; adaptive defaults apply until the new settings are saved.
