...
Extraction serves each document's **complete source pages in one payload**. Page-intact adaptive windows (hearings 6,000 characters/six pages; reports 10,000/10; minute orders 6,000/6; all under the 12,000-character safety limit, examination breaks preferred for hearings) remain only for the direct-API paths — the prompt-testing window and minute-order summaries — where each window is a separate request. PI extraction is not windowed: the hearing payload privately includes validated participant-index context under `PARTICIPANT INDEX CONTEXT — FOR ATTRIBUTION ONLY` (attribution context only; every fact still needs evidence from the original hearing pages).
